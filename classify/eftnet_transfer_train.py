...
def train(opt, device):
    ...
    ##>>>> transfer learning based on efficientnet b0 in pytorch
    ## note: we freeze all params in model except for build a new output fully connected layer
    LOGGER.info("NOTE: in transfer learning mode !!! based on arch: {}".format(model.__class__.__name__))
    
    for p in model.parameters():
        p.requires_grad = False  # for transfer training
    n_inputs = model.classifier[1].in_features
    n_outputs = model.classifier[1].out_features
    ## note: the hyper param 2048 can be replaced
    model.classifier = nn.Sequential(
        nn.Linear(n_inputs,2048),
        nn.SiLU(),
        nn.Dropout(0.2),
        ## note: explict bias = True
        nn.Linear(2048, n_outputs, bias=True), 
    )
    ##>>>> compatibility convert for new modules
    for m in model.modules():
        t = type(m)
        if t in (nn.Hardswish, nn.LeakyReLU, nn.ReLU, nn.ReLU6, nn.SiLU):
            m.inplace = True ## note: hardcoded now
        elif t is nn.Upsample and not hasattr(m, "recompute_scale_factor"):
            m.recompute_scale_factor = None  # torch 1.11.0 compatibility
            
    LOGGER.info("new classifier layer: {}".format(model.classifier))
