...
在扶摇（Fuyao）或本地构建 YOLOv5 检测训练镜像。镜像内通过 **正式 `pip install .`** 安装 YOLOv5（非 editable），并预装 tensorboard、ossutil；**不安装 wandb**。
...
| 脚本 | 用途 | 命令 |
|------|------|------|
| [`build.sh`](build.sh) | **本地** `docker build`（NVCR 镜像 + `--build-arg BASE_IMAGE`） | `bash docker/build.sh` |
| [`push_fuyao.sh`](push_fuyao.sh) | **扶摇远程** `fuyao docker --push`（Dockerfile 默认 fuyao-image-convert） | `bash docker/push_fuyao.sh` |

二者互不调用，无第三份 wrapper。构建前若缺少 ossutil zip，各自独立下载。
...
| 本地 build（`build.sh` 传入） | `ywvk8934o3f50v3q9i-nvcr.xuanyuan.run/nvidia/pytorch:25.02-py3` |

扶摇使用 **fuyao-image-convert**（PyTorch 2.11），与 [`species_recognition/docker/fuyao_vfm_apple_v0.0.2.dockerfile`](../../../species_recognition/docker/fuyao_vfm_apple_v0.0.2.dockerfile) 一致。

## 与 vfm_apple 扶摇构建的差异

| 项目 | vfm_apple（yolo26） | YOLOv5 本镜像 |
|------|---------------------|---------------|
| 典型 cwd | `species_recognition/docker/` | **`ultralytics_yolov5/` 仓库根** |
| 是否 COPY 业务源码 | 否（仅 pip 装 ultralytics） | **是**（`COPY . /tmp/yolov5` → `pip install .`） |
...
构建时源码处理：`COPY . /tmp/yolov5` → `install_yolov5.sh` 正式安装 → 删除 `/tmp/yolov5`。训练 Job 使用镜像内 site-packages 中的 YOLOv5，不依赖 Job 再打包一份源码。
...
# IMAGE_TAG=yolov5_train-v0.0.2 bash docker/push_fuyao.sh
```
...
```bash
fuyao docker --push \
  --dockerfile="./docker/fuyao_yolov5_v0.0.1.dockerfile" \
  --image-tag "yolov5_train-v0.0.1"
```
...
| 文件 | 说明 |
|------|------|
| [`fuyao_yolov5_v0.0.1.dockerfile`](fuyao_yolov5_v0.0.1.dockerfile) | 主 Dockerfile |
| [`build.sh`](build.sh) | 本地 docker build |
...
A: 只用 `bash docker/build.sh`，已内置 NVCR 本地镜像 URL。
